...
struct Plane
{
    ...
    //bool FitToPoints(const std::vector<Vec3f> &Points, float &ResidualError);
    //bool FitToPoints(const std::vector<Vector4f> &Points, Vec3f &Basis1, Vec3f &Basis2, float &NormalEigenvalue, float &ResidualError);
    ...
    //static float Dot(const Plane &P, const Vector4f &V);            //dot product of a plane and a 4D vector
